...
const FaqPage = () => {
  return (
    <div>
      ...
      <FaqItem question="Can I use these assets commercially?">
      ...
      <FaqItem question="Why do diffuse maps distributed on this website lack any form of shading?">
        <p>These diffuse textures are actually <em>albedo</em> maps, designed for use in physically-based rendering engines. We use the term "diffuse" instead because it's a little more widely understood than "albedo", and a little more specific than "color".</p>
        <p>For PBR rendering to look correct, albedo maps should not contain any shading. Instead, shading is applied by the rendering engine itself, with the help of physical geometry displacement and path traced shading, or a separate ambient occlusion map. Without real micro-displacement geometry, the separate ambient occlusion map can be used to simulate the effect that the real geometry would have on the shading in an average environment, typically only applied to indirect lighting.</p>
        <p>This is why we remove as much shading from our diffuse maps as possible, though we can't always remove everything. In some cases, it's desirable to leave in microscopic shading which would not be reproduced by shading real geometry displacement anyway.</p>
        <p>By contrast, diffuse maps used in older or non-photorealistic rendering engines are intended to contain shading. If you are working with such a rendering engine, or if you wish to reduce the distribution file size of your project, you can merge the albedo and AO maps together to create a traditional diffuse map. This can be done by multiplying the AO map on top of the albedo map in an image editor.</p>
      </FaqItem>
