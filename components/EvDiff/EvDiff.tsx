...
const EvDiff = () => {
  ...
  const calcEvs = () => {
    ...
    const shouldnt_be_empty = [a_shutter,
      b_shutter,
      a_aperture,
      b_aperture,
      a_iso,
  ...
  return (<div className={styles.wrapper}>
    ...
    <form onSubmit={e => e.preventDefault()}>
      <div className={styles.formItem}>
        <h2>Image A: </h2>
        <input type="text" value={shutterA} onChange={e => set_shutterA(e.target.value)} placeholder="Shutter Speed" />
        ...
        <input type="text" value={filterA} onChange={e => set_filterA(e.target.value)} placeholder="ND Filter EVs" />
        <br />
      </div>
      <div className={styles.formItem}>
        <h2>Image B:</h2>
        <input type="text" value={shutterB} onChange={e => set_shutterB(e.target.value)} placeholder="Shutter Speed" />
        ...
        <input type="text" value={filterB} onChange={e => set_filterB(e.target.value)} placeholder="ND Filter EVs" />
      </div>
      <div className={styles.formItem}>
        {message === "Complete" ? (
          result >= 0 ?
            <h2>Image A is <pre>{Math.abs(result)}</pre> EVs darker than Image B.</h2>
            :
            <h2>Image A is <pre>{Math.abs(result)}</pre> EVs brighter than Image B.</h2>
        ) : <h2>{message}</h2>}
      </div>
    </form>
