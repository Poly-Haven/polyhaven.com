...
const GallerySubmit = ({ assets, galleryApiUrl }) => {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <p>
        <Trans i18nKey="submit.s1p1" t={t} components={{ lnk: <LinkText href="/gallery" /> }} />
      </p>
      <p>{t('submit.s1p2')}</p>
      <p>
        <Trans i18nKey="submit.s1p3" t={t} components={{ em: <em /> }} />
      </p>

      <h2>{t('submit.s2')}</h2>
      <ol>
        <li>
          <Trans i18nKey="submit.s2p1" t={t} components={{ strong: <strong /> }} />
        </li>
        <li>
          <Trans i18nKey="submit.s2p2" t={t} components={{ strong: <strong /> }} />
        </li>
        <li>{t('submit.s2p3')}</li>
      </ol>
      ...
      <h2>{t('submit.s3')}</h2>
      <p>
        <Trans i18nKey="submit.s3p1" t={t} components={{ strong: <strong /> }} />
      </p>
      <p>{t('submit.s3p2')}</p>
      <p>
        <Trans i18nKey="submit.s3p3" t={t} components={{ lnk: <a href="https://discord.gg/Dms7Mrs" /> }} />
      </p>
      <p>{t('submit.s3p4')}</p>
      <p>{t('submit.s3p5')}</p>
      <ul>
        ...
        <li>
          <Trans i18nKey="submit.s3p5l6" t={t} components={{ strong: <strong /> }} />
        </li>
      </ul>
        ...
        >
          ...
          <div className={styles.buttonWrapper}>
            <label
              ...
              className={`${btnStyles.button} ${btnStyles[image ? 'hollow' : 'accent']} ${styles.imageSelect}`}
            >
              {image ? t('submit.form.file-change') : t('submit.form.select-change')}
              <br />
              <span className={styles.sub}>{t('submit.form.file-size')}</span>
            </label>
          </div>
          <input
            type="file"
          ...
          <GalleryFormItem label={t('submit.form.art-name')} optional={true}>
