...
const Popularity = ({ data }) => {
          ...
          <table>
            <tr>
              ...
              {data[month].bonus_usd_rate ? (
                <th>
                  Bonus (USD){' '}
                  <abbr
                    title={`Using exchange rate on date of bonus payment: $1 = ${money(data[month].bonus_usd_rate)}`}
                  >
                    <MdHelp />
                  </abbr>
                </th>
            ...
            {Object.keys(popData.author_totals).map((author, i) => (
              <tr>
                ...
                {data[month].bonus_usd_rate ? (
                  <td>{getCurrency(popData.author_totals[author], 'USD', { USD: data[month].bonus_usd_rate })}</td>
                ) : null}
              </tr>
            ))}
          </table>
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <p>
        The popularity rating* is used to determine the bonus earnings for the author of each HDRI. More popular HDRIs
        earn the author more income (on top of the base rate).
      </p>

      <p>
        The purpose of rewarding more popular HDRIs is to encourage HDRI contributions that are more useful or more
        wanted by users.
      </p>

      <p>
        The total budget allocation for HDRIs this month is: <strong>{money(budget)}</strong> (25% of Patreon +
        Advertising <Link href="/finance-reports">income</Link>).
      </p>

      <p>
        After subtracting the up-front payments** for these HDRIs, the remainder to share for bonus earnings is:{' '}
      ...
      <div className={styles.footnotes}>
        <p>
          * Popularity rating is determined by unique downloads over the first 28 days of the HDRI's publication, with
          some basic statistical maniplulation to lower the influence of large spikes and troughs in downloads.
          <br />
          This is to account for inconsistent social media influence, public holidays, and server instabilities, and
          thus attempt to provide a fairer comparison of quality and usefulness between HDRIs.
          <br />
        ...
        <p>
          ** $100 per HDRI, or $140 if it includes backplates (max 1 per author). Only one HDRI per month may earn a 40%
          bonus for including backplates - if more than one includes backplates, the most popular HDRI is preferred.
        </p>
