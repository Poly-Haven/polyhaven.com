...
const PatronCounts = ({ data }: { data: PatronData }) => {
  ...
  const defaultStartIndex = Math.max(0, graphData.length - 365)
  ...
  const visibleNoteEntries = Object.entries(notes)
    .map(([date, label], i) => ({ date, label, num: i + 1 }))
    .filter(({ date }) => {
      const idx = graphData.findIndex((d) => d.date === date)
      return idx !== -1 && idx >= brushRange.startIndex && idx <= brushRange.endIndex
    })
        ...
        <ResponsiveContainer>
          <LineChart data={graphData}>
            ...
            <Line
              ...
              connectNulls={true}
            />
            {visibleNoteEntries.map(({ date, num }) => (
              <ReferenceLine
                key={date}
                x={date}
                stroke="rgba(255, 70, 70, 0.75)"
                label={<Label value={num} position="insideTopLeft" fill="rgba(255, 70, 70, 0.75)" />}
              />
            ))}
          </LineChart>
        </ResponsiveContainer>
      ...
      {visibleNoteEntries.length > 0 ? (
        <>
          ...
          <ol className={styles.notesList}>
            {visibleNoteEntries.map(({ date, label, num }) => (
              <li key={date} style={{ counterReset: `count ${num - 1}` }}>
